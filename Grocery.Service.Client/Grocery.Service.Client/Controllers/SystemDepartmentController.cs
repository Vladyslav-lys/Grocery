...
{
    class SystemDepartmentController
    {
        private SystemSettings systemSettings;
        private IFrontServiceClient frontServiceClient;

        public SystemDepartmentController(SystemSettings systemSettings, IFrontServiceClient frontServiceClient)
        {
            this.systemSettings = systemSettings;
            this.frontServiceClient = frontServiceClient;
        }

        public async Task<List<Department>> ShowDepartmentsAsync()
        {
            try
            {
                OperationStatusInfo operationStatusInfo = await this.systemSettings.Connection.InvokeAsync<OperationStatusInfo>("ShowDepartment");
                ...
                if (operationStatusInfo.OperationStatus == OperationStatus.Done)
                {
                    ...
                    List<Department> departments = JsonConvert.DeserializeObject<List<Department>>(attachedObjectText);
