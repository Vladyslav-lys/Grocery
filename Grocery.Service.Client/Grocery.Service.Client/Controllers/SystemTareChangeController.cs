...
{
    class SystemTareChangeController
    {
        private SystemSettings systemSettings;
        private IFrontServiceClient frontServiceClient;

        public SystemTareChangeController(SystemSettings systemSettings, IFrontServiceClient frontServiceClient)
        {
        ...
        public async Task<List<TareChange>> ShowTareChangeAsync()
        {
            try
            {
                OperationStatusInfo operationStatusInfo = await this.systemSettings.Connection.InvokeAsync<OperationStatusInfo>("ShowTareChanges");
                ...
                if (operationStatusInfo.OperationStatus == OperationStatus.Done)
                {
                    ...
                    List<TareChange> tareChanges = JsonConvert.DeserializeObject<List<TareChange>>(attachedObjectText);
